...
class VectorStore:
    ...
    def search(
        self, query_embedding: np.ndarray, top_k: int = 5, filter_metadata: Dict[str, Any] = None
    ) -> List[Dict[str, Any]]:
        """搜索最相似的文档"""
        raise NotImplementedError
...
class ChromaStore(VectorStore):
    ...
    def __init__(
        self, persist_directory: str = "./data/chroma_db", collection_name: str = "knowledge_base"
    ):
    ...
    def _init_collection(self):
        ...
        try:
            ...
            try:
                ...
                _logger.warning(
                    "PersistentClient 初始化失败,降级 EphemeralClient(重启会丢): %s",
                    e,
                )
            ...
            self._collection = self._client.get_or_create_collection(
                name=self.collection_name, metadata={"description": "知识库向量存储"}
            )
            print(
                f"[OK] ChromaDB 集合 '{self.collection_name}' 初始化完成 (path={self.persist_directory}, persistent={self._is_persistent})"
            )
    ...
    def _search_inmemory(self, query_embedding: np.ndarray, top_k: int = 5) -> List[Dict[str, Any]]:
        """内存搜索（降级方案）- 使用余弦相似度"""
...
class FAISSStore(VectorStore):
    ...
    def __init__(
        self, index_path: str = "./data/faiss.index", docs_path: str = "./data/faiss_docs.json"
    ):
    ...
    def save(self):
        ...
        with open(self.docs_path, "w", encoding="utf-8") as f:
            json.dump(
                [asdict(d) for d in self._documents.values()], f, ensure_ascii=False, default=str
            )
