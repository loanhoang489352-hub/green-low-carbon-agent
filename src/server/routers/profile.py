...
def register_profile_routes(registry) -> None:
    ...
    # P6.A: profile/personalization/stats/conversation 全部需鉴权(用户隐私数据)
    registry.add_route("GET", "^/api/profile/", profile_get, auth_required=True, description="用户画像(GET)")
    registry.add_route("GET", "^/api/personalization/", personalization_get, auth_required=True, description="个性化上下文(GET)")
    registry.add_route("POST", "/api/personalization/context", personalization_context, auth_required=True, description="个性化上下文(POST)")
    registry.add_route("GET", "^/api/stats/", user_stats, auth_required=True, description="用户统计")
