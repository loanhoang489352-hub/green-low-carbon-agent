...
def register_feedback_routes(registry) -> None:
    ...
    def feedback_history(handler, data):
        ...
        user_id = parts[-1] if len(parts) > 3 and parts[-1] != "history" else data.get("user_id", "anonymous")
        try:
    ...
    registry.add_route("POST", "/api/feedback", feedback_submit, auth_required=False, description="提交反馈")
    registry.add_route("POST", "/api/feedback/message", feedback_message, auth_required=False, description="消息反馈详情")
    registry.add_route("POST", "/api/feedback/stats", feedback_stats, auth_required=False, description="反馈统计")
    registry.add_route("POST", "/api/feedback/history", feedback_history, auth_required=False, description="用户反馈历史")
    registry.add_route("POST", "/api/feedback/negative", feedback_negative, auth_required=False, description="最近负面反馈")
