...
def register_feedback_routes(registry) -> None:
    ...
    registry.add_route("POST", "/api/feedback", feedback_submit, auth_required=True, description="提交反馈")
    registry.add_route("POST", "/api/feedback/message", feedback_message, auth_required=True, description="消息反馈详情")
    registry.add_route("POST", "/api/feedback/stats", feedback_stats, auth_required=True, description="反馈统计")
    registry.add_route("POST", "/api/feedback/history", feedback_history, auth_required=True, description="用户反馈历史")
    registry.add_route("POST", "/api/feedback/negative", feedback_negative, auth_required=True, description="最近负面反馈")
