...
class PolicyUpdater:
    ...
    def add_policy(
        ...
        try:
            ...
            """, (
                policy_id, title, content, category, source, source_url,
                publish_date, summary, json.dumps(key_points, ensure_ascii=False),
                impact_level, content_hash, now, now
            ))
    ...
    def export_to_markdown(self, output_path: str = None) -> str:
        ...
        if output_path is None:
            output_path = self.knowledge_base_path / f"policy_update_{datetime.now().strftime('%Y%m%d')}.md"
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_checks": [
                {"source": row[0], "last_checked": row[1]}
                for row in recent_checks[:5]
            ]
        }
        ...
        sample_policies = [
            {
                ...
                "key_points": ["形成绿色低碳生活方式", "推动大规模设备更新", "完善绿色低碳发展体系"],
                "impact_level": "high"
            },
            {
                ...
                "key_points": ["补贴标准根据续航里程确定", "私人购车补贴延续", "充电基础设施建设支持"],
                "impact_level": "high"
            },
            {
                "title": "绿色建材下乡活动实施方案",
