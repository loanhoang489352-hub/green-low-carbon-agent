...
class AchievementSystem:
    ...
    def _check_badges(self, user_id: str) -> List[str]:
        ...
        for badge in self.BADGES:
            ...
            if "action_count" in req:
                ...
                travel_actions = [a for a in actions 
                             if a["action"] in ["公共交通1次", "步行/骑行1次", "拼车1次"]]
    ...
    def get_user_profile(self, user_id: str) -> Dict:
        ...
        # 等级信息
        level_info = self.LEVELS[user.level - 1] if user.level <= len(self.LEVELS) else self.LEVELS[-1]
    ...
    def get_all_badges(cls) -> List[Dict]:
        """获取所有徽章"""
        return [{"id": b["badge_id"], "name": b["name"], "icon": b["icon"]} 
                for b in cls.BADGES]